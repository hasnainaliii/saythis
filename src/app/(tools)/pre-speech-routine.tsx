...
const INTRO_CARDS = [
  ...
  {
    step: 2, icon: 'list-outline', iconColor: ACCENT,
  ...
  {
    step: 3, icon: 'repeat-outline', iconColor: ACCENT,
...
export default function PreSpeechRoutineScreen() {
  ...
  const getStepDurations = () => {
    const names = ['settle', 'release', 'warmUp', 'easyOnset', 'affirm'] as const;
      ...
      <>
        ...
        <View style={{ flex: 1, backgroundColor: colors.primary, paddingTop: insets.top }}>
          ...
          <ScrollView style={{ flex: 1 }}>
            <Animated.View key={currentStep} entering={SlideInRight.duration(300)} exiting={SlideOutLeft.duration(300)}>
              ...
              {currentStep === 3 && <EasyOnsetStep onComplete={advanceStep} phrases={prespeechPhrases} />}
              {currentStep === 4 && <AffirmStep onComplete={advanceStep} affirmation={todayAffirmation} />}
            </Animated.View>
          </ScrollView>
        </View>
      </>
  ...
  return (
    <>
      ...
      <ToolResultScreen
        ...
        tipCard={{
          icon: 'rocket-outline', iconColor: ACCENT,
          title: 'Now go do it',
        ...
        onSave={handleSave}
        onRepeat={() => { setStage(0); setCurrentStep(0); }}
      />
    </>
  );
}
